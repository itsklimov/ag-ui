...
def _required_key_text(key: Any) -> str:
    ...
    full. They do not agree on every number. Python prints large and tiny
    magnitudes in full where JavaScript switches to an exponent at 1e21 and
    below 1e-6, pads to two exponent digits where JavaScript writes one, and
    prints the non-finite floats Python's ``json`` accepts by default as
    ``nan`` and ``inf`` where JavaScript writes ``NaN`` and ``Infinity``.
    """
        ...
        *,
        ...
        if self._orchestrator is not None:
            ...
            if orchestrator_thread in self._active_orchestrator_runs or blocked_by_park:
                ...
                yield RunErrorEvent(
                    type=EventType.RUN_ERROR,
                    # Written inline rather than through a variable so both
                    # renderings stay pinned by the error-code contract. No run
                    # is in progress on the parked branch, so the busy wording
