...
fails, it MUST fail the same way.
...
something the boundary can no longer recognise. Middleware a consumer installs
itself also sees chunks unexpanded, so a middleware that rewrites message
content has to handle the chunk form as well as the start/content/end form the
rest of this document is written in.

Enforcement MUST run before chunk expansion. A chunk is an event in its own
right, so it is validated as one like anything else, and expansion then only
reshapes values already known good. Expanding first gives the expander
unvalidated input to cope with, and coping means repairing: a consumer that
substituted a default for a malformed role made the same producer defect fatal
when sent plainly and invisible when sent as a chunk, so which form a producer
happened to use decided whether its bug was ever reported.
